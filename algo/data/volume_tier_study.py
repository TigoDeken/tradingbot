"""
Signal study across volume tiers: funding rate, OI, and relative volume
as predictors of 1H forward returns.

Symbols chosen to represent ~$200M, ~$50M, ~$10M, ~$1M, ~$300K daily volume tiers.
Same methodology as signal_study.py (BTC baseline).

Run: python -m algo.data.volume_tier_study
"""
...
OUTPUT    = os.path.join(CACHE_DIR, "volume_tier_study_output.txt")
HORIZONS  = [4, 24, 168]
MIN_EDGE  = 0.40
N_PERMS   = 1000
...
def fetch_ohlcv(symbol: str) -> pd.DataFrame:
    _print(f"  [{symbol}] Fetching 1H OHLCV...")
    bars, end = [], None
    while True:
...
def fetch_funding(symbol: str) -> pd.DataFrame:
    _print(f"  [{symbol}] Fetching funding rate...")
    bars, end = [], None
    while True:
...
def fetch_oi(symbol: str) -> pd.DataFrame:
    _print(f"  [{symbol}] Fetching open interest...")
    bars, end = [], None
    while True:
...
def build(ohlcv, funding, oi) -> pd.DataFrame:
    ...
    df["oi_chg_72h"]     = df["oi"].pct_change(72) * 100
    oi_roll              = df["oi_chg_24h"].rolling(2160)
    df["oi_zscore"]      = (df["oi_chg_24h"] - oi_roll.mean()) / oi_roll.std()
    df["above_200d"]     = df["close"] > df["close"].rolling(4800).mean()

    for h in HORIZONS:
        df[f"fwd_{h}h"] = (df["close"].shift(-h) / df["close"] - 1) * 100

    required = ["rel_vol","funding_zscore","oi_chg_24h","oi_chg_72h","oi_zscore"] + [f"fwd_{h}h" for h in HORIZONS]
...
def analyse_signal(df, signal, label):
    ...
    for regime_label, mask in [
        ...
        for h in HORIZONS:
            ...
            _print(f"      {h:>4}h: IC={ic:+.4f}  p(param)={pval:.3f}  "
...
def run_symbol(symbol: str, tier: str):
    ...
    if os.path.exists(cache):
        _print(f"  Loading cache...")
        df = pd.read_parquet(cache)
        if "oi_chg_72h" not in df.columns:
            _print(f"  Migrating cache: adding oi_chg_72h and oi_zscore...")
            df["oi_chg_72h"] = df["oi"].pct_change(72) * 100
            oi_roll = df["oi_chg_24h"].rolling(2160)
            df["oi_zscore"] = (df["oi_chg_24h"] - oi_roll.mean()) / oi_roll.std()
            df = df.dropna(subset=["oi_chg_72h", "oi_zscore"])
            df.to_parquet(cache)
    else:
        try:
    ...
    _print(f"  Dataset: {len(df)} rows  |  "
           f"{df.index[0].date()} to {df.index[-1].date()}  |  "
           f"Bull: {df['above_200d'].sum()}  Bear: {(~df['above_200d']).sum()}")

    for col, label in [
        ("oi_chg_24h",     "OI 24h change (%)"),
        ("oi_chg_72h",     "OI 72h change (%)"),
        ("oi_zscore",      "OI Z-Score (90d, based on 24h change)"),
...
if __name__ == "__main__":
    ...
    with open(OUTPUT, "w") as _out_file:
        _print("Volume Tier Signal Study")
        _print(f"Symbols: {', '.join(SYMBOLS.keys())}")
        _print(f"Signals: OI 24h change, Funding Rate, Funding Z-Score, Relative Volume")
        _print(f"Horizons: {HORIZONS}h  |  Min edge: {MIN_EDGE}%  |  Perms: {N_PERMS}")

        for symbol, tier in SYMBOLS.items():
            run_symbol(symbol, tier)
